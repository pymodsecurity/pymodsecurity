...
void init_rule_message(py::module &m)
{
    py::class_<RuleMessage>(m, "RuleMessage")
        ...
        .def("log", py::overload_cast<>(&RuleMessage::log))
        .def("log", py::overload_cast<int>(&RuleMessage::log))
        .def("log", py::overload_cast<int, int>(&RuleMessage::log))
        .def("errorLog", &RuleMessage::errorLog)
        .def_static("slog", py::overload_cast<const RuleMessage *, int, int>(&RuleMessage::log))
        .def_static("slog", py::overload_cast<const RuleMessage *, int>(&RuleMessage::log))
        .def_static("slog", py::overload_cast<const RuleMessage *>(&RuleMessage::log))
        .def_static("_details", py::overload_cast<const RuleMessage *>(&RuleMessage::_details))
        .def_static("_errorLogTail", py::overload_cast<const RuleMessage *>(&RuleMessage::_errorLogTail))
