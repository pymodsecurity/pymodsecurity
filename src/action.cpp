...
void init_action(py::module &m)
{
    ...
    action.def(py::init<const std::string&>())
        ...
        .def("evaluate", py::overload_cast<Rule*, Transaction*>(&Action::evaluate))
        .def("evaluate", py::overload_cast<Rule*, Transaction*, std::shared_ptr<RuleMessage>>(&Action::evaluate))
